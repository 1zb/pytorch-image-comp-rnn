...
def save(index, epoch=True):
    ...
    torch.save(
        encoder.state_dict(), "checkpoint/encoder_{}_{:08d}.pth".format(s, index)
    )

    torch.save(
        binarizer.state_dict(), "checkpoint/binarizer_{}_{:08d}.pth".format(s, index)
    )

    torch.save(
        decoder.state_dict(), "checkpoint/decoder_{}_{:08d}.pth".format(s, index)
    )
...
for epoch in range(last_epoch + 1, args.max_epochs + 1):
    ...
    for batch, data in enumerate(train_loader):
        ...
        print(
            "[TRAIN] Epoch[{}]({}/{}); Loss: {:.6f}; Backpropagation: {:.4f} sec; Batch: {:.4f} sec".format(
                epoch,
                batch + 1,
                len(train_loader),
                loss.data[0],
