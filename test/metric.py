...
def msssim(original, compared, crop_original=False):
    ...
    if crop_original:
        original = original[:, : compared.shape[1], : compared.shape[2], :]
...
def main():
    ...
    parser.add_argument(
        ...
        "-crop",
        action="store_true",
        help="crop original to match comaparable image's shape",
    )
    args = parser.parse_args()

    if args.metric != "psnr":
        print(
            msssim(args.original_image, args.compared_image, args.crop_original), end=""
        )
    if args.metric != "ssim":
